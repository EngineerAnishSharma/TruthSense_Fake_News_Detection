...
export function ThemeToggle() {
  ...
  const [themeColor, setThemeColor] = useState("var(--color-one)");
  const [themeLightColor, setThemeLightColor] = useState("var(--color-one-light)");
  const [themehsl, setThemehsl] = useState("238.8, 100%, 70.59%");
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (storedThemeLightColor) setThemeLightColor(storedThemeLightColor);
      if (storedThemeHSLColor) setThemehsl(storedThemeHSLColor);
    }
  }, []);

  const changeColor = (theme: string, themeLight: string, themehsl: string): void => {
    setThemeColor(theme);
  ...
  }, [theme, themeColor, themeLightColor]);
  ...
  return (
    <div className={cn("flex items-center justify-start gap-2 group/sidebar py-2")}>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="center" className="flex justify-center gap-x-1.5 py-2">
          <div className="rounded-full bg-colorOne w-5 h-5" onClick={() =>
            changeColor("var(--color-one)", "var(--color-one-light)", "var(--color-one-hsl)")
          }></div>
          <div className="rounded-full bg-colorTwo w-5 h-5" onClick={() =>
            changeColor("var(--color-two)", "var(--color-two-light)", "var(--color-two-hsl)")
          }></div>
          <div className="rounded-full bg-colorThree w-5 h-5" onClick={() =>
            changeColor("var(--color-three)", "var(--color-three-light)", "var(--color-three-hsl)")
          }></div>
          <div className="rounded-full bg-colorFour w-5 h-5" onClick={() =>
            changeColor("var(--color-four)", "var(--color-four-light)", "var(--color-four-hsl)")
          }></div>
          <div className="rounded-full bg-colorFive w-5 h-5" onClick={() =>
            changeColor("var(--color-five)", "var(--color-five-light)", "var(--color-five-hsl)")
          }></div>
        </DropdownMenuContent>
      </DropdownMenu>
